...
import realtimePlugin, { noopRealtime } from './plugins/realtime.js'
...
export interface BuildAppOptions {
  /**
   * เปิด WebSocket (`/ws`) หรือไม่
   * บน serverless ต้องปิด เพราะโปรเซสไม่ได้อยู่ค้างเพื่อถือ connection ไว้
   */
  realtime?: boolean
  /** จำกัดอัตราการเรียก — เก็บสถานะในหน่วยความจำ จึงมีผลเฉพาะตอนรันเป็นเซิร์ฟเวอร์เดียว */
  rateLimit?: boolean
}

export async function buildApp(options: BuildAppOptions = {}): Promise<FastifyInstance> {
  const { realtime = true, rateLimit: enableRateLimit = true } = options
  ...
  await app.register(realtime ? realtimePlugin : noopRealtime)
